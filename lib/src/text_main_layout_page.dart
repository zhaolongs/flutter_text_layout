...
class _PageState extends State<TextLayoutMainPage> {
  ...
  Widget buildBody3() {
    ...
    for (TextLayoutModel textModel in widget.textLayoutModelList) {
      ...
      ///显示文字的颜色
      Color fontColor = (textModel.textColor==null||textModel.isTextColorRandom)?getRandomColor():textModel.textColor;
      ///背景颜色
      ...
      if(textModel.isShowCirPoint){
       cirPoint =Container(
         ...
         decoration: BoxDecoration(
             ...
             boxShadow: [
               BoxShadow(color: iconColor, offset: Offset(radius/10, radius/4),blurRadius: 0.2),
               BoxShadow(color: iconColor, offset: Offset(-radius/10, radius/4),blurRadius: 0.2)
             ]),
       );
      }
      ...
      Row row = Row(
       ...
       switch(backgroundModel){
         ...
         backGroundRadius =
             (10 + fontSize + widget.padding.top + widget.padding.bottom +
                 widget.margins.top + widget.margins.bottom) / 2;
           break;
      ...
      switch(backgroundModel){
        ...
        case TextLayoutBackgroundModel.rrectBorder:
        case TextLayoutBackgroundModel.ovalBorder:
         border = textModel.border??new Border.all(color:backgroundColor, width: 0.5);
    ...
    return Container(
      ...
      child: Wrap(spacing: 8.0, runSpacing: 8.0, children: columnList,alignment: WrapAlignment.spaceEvenly,),
    );
  }
